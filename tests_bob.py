...
if __name__ == '__main__':
    ...
    data = Extract('./sensors_bobtests.yaml')
    ...
    for sensor in data.sensor.keys():
        for i in range(1):
            ...
            # Apply RANSAC if stated in yaml
            try:
                try:
                    data.sensor[sensor]['init_guess'] = perform_RANSAC(data.sensor[sensor]['pc_parent_crop'], icp_info[sensor]['pc_target_parent'], visualize=False, voxelsize= data.sensor[sensor]['ransac']['voxelsize'])
                except:
                    data.sensor[sensor]['init_guess'] = perform_RANSAC(data.sensor[sensor]['pc_parent'], icp_info[sensor]['pc_target_parent'], visualize=False, voxelsize= data.sensor[sensor]['ransac']['voxelsize'])
            except:
                pass
            ...
            try:
                ...
                icp_info[sensor]['correction'], icp_info[sensor]['rmse'] = perform_icp(data.sensor[sensor]['pc_parent_crop'], icp_info[sensor]['pc_target_parent'], data.sensor[sensor]['init_guess'])
            except:
                o3d.visualization.draw_geometries([data.sensor[sensor]['pc_parent'] + icp_info[sensor]['pc_target_parent']])
                icp_info[sensor]['correction'], icp_info[sensor]['rmse'] = perform_icp(data.sensor[sensor]['pc_parent'], icp_info[sensor]['pc_target_parent'],  data.sensor[sensor]['init_guess'])
