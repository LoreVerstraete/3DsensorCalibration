...
if __name__ == '__main__':
    ...
    # Read the overlap input
    with open('./overlap.yaml', 'r') as file:
    ...
    # Transform target to the source parent
    icp_info[sensor]['pc_target_parent'] = copy_pointcloud(data.sensor[extra['target']]['pc_base'])
    icp_info[sensor]['pc_target_parent'].transform(data.sensor[sensor]['transformbaseparent'])

    # Apply ICP preferably with a cropped source
    try:
